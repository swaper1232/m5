...
// Используем только функции из DeviceLockState.h вместо собственных версий
//bool getLockState(const String& deviceAddress);
//bool getLockState();
//void setLockState(const String& deviceAddress, bool locked);
//void setLockState(bool locked);
// Новые функции для работы с состоянием блокировки устройств
...
void saveDeviceSettings(const String& deviceAddress, const DeviceSettings& settings) {
    ...
    err = nvs_set_i8(nvsHandle, isLockedKey.c_str(), settings.isLocked ? 1 : 0);
    if (err != ESP_OK) {
        Serial.printf("Error saving lock status: %d\n", err);
    }
...
DeviceSettings getDeviceSettings(const String& deviceAddress) {
    ...
    settings.isLocked = false; // По умолчанию устройство не заблокировано
    ...
    int8_t locked;
    if (nvs_get_i8(nvsHandle, ("locked_" + shortKey).c_str(), &locked) == ESP_OK) {
        settings.isLocked = (locked == 1);
    }
...
void checkLockState() {
    ...
    // Проверяем статус последнего заблокированного устройства
    char lastAddr[32] = {0};
    size_t length = sizeof(lastAddr);
    
    if (nvs_get_str(nvsHandle, KEY_LAST_ADDR, lastAddr, &length) == ESP_OK && strlen(lastAddr) > 0) {
        Serial.printf("Last locked device: %s\n", lastAddr);
        
        // Проверяем, заблокировано ли оно еще
        bool isLocked = getDeviceLockState(String(lastAddr));
        Serial.printf("Lock state: %s\n", isLocked ? "LOCKED" : "UNLOCKED");
    } else {
        Serial.println("No last locked device found");
    }
    
    // Проверяем статус текущего подключенного устройства, если оно отличается от последнего
    if (connectedDeviceAddress.length() > 0 && 
        (strlen(lastAddr) == 0 || strcmp(connectedDeviceAddress.c_str(), lastAddr) != 0)) {
        
        Serial.printf("Current connected device: %s\n", connectedDeviceAddress.c_str());
        bool isLocked = getDeviceLockState(connectedDeviceAddress.c_str());
        Serial.printf("Lock state: %s\n", isLocked ? "LOCKED" : "UNLOCKED");
    }
    
    Serial.println("=== End Checking Lock State ===\n");
}
...
void setup() {
    ...
    if (nvs_get_str(nvsHandle, KEY_LAST_ADDR, lastAddr, &length) == ESP_OK && strlen(lastAddr) > 0) {
        String lastDevice = String(lastAddr);
        bool wasLocked = getDeviceLockState(lastDevice);
        
        if (wasLocked) {
            currentState = LOCKED;
            connectedDeviceAddress = lastAddr; // Используем char* вместо String
            Serial.println("\n=== Previous state was LOCKED ===");
            Serial.printf("Last locked device: %s\n", lastDevice.c_str());
            
            // Если устройство было заблокировано, но мы переподключились,
            // попробуем автоматически разблокировать его при восстановлении соединения
            if (serialOutputEnabled) {
...
void loop() {
    ...
    if (currentState == LOCKED) {
                // Сначала проверяем, что текущее устройство было заблокировано
                bool isCurrentDeviceLocked = getDeviceLockState(connectedDeviceAddress.c_str());
                
                // Разблокировываем только если устройство было заблокировано
                if (isCurrentDeviceLocked) {
                    ...
                    // Выводим сообщение, что текущее устройство не является тем, которое было заблокировано
                    Serial.println("Current device is not the one that was locked. Unlock not attempted.");
                }
            }
        }
    }
...
void lockComputer() {
    ...
    if (success) {
        // Сохраняем состояние блокировки для текущего устройства
        setDeviceLockState(connectedDeviceAddress.c_str(), true);
        Serial.printf("Lock state set for device: %s\n", connectedDeviceAddress.c_str());
    }
...
void unlockComputer() {
    ...
    for(int attempt = 0; attempt < 3 && !success; attempt++) {
        ...
        if (success) {
            ...
            // 3. Сбрасываем состояние блокировки для конкретного устройства
            setDeviceLockState(connectedDeviceAddress.c_str(), false);
...
// Функция для получения состояния блокировки конкретного устройства
bool getDeviceLockState(const String& deviceAddress) {
    DeviceSettings settings = getDeviceSettings(deviceAddress);
    return settings.isLocked;
}

// Перегрузка функции для получения статуса текущего подключенного устройства
bool getDeviceLockState() {
    if (connectedDeviceAddress.length() == 0) {
...
// Функция для установки состояния блокировки конкретного устройства
void setDeviceLockState(const String& deviceAddress, bool locked) {
    DeviceSettings settings = getDeviceSettings(deviceAddress);
...
// Перегрузка функции для установки статуса текущего подключенного устройства
void setDeviceLockState(bool locked) {
    esp_err_t err = nvs_set_i8(nvsHandle, KEY_IS_LOCKED, locked ? 1 : 0);
    if (err != ESP_OK) {
        Serial.printf("Error setting lock state: %d\n", err);
...
// Функция для установки глобального состояния блокировки (старая версия)
void setGlobalLockState(bool locked) {
    esp_err_t err = nvs_set_i8(nvsHandle, KEY_IS_LOCKED, locked ? 1 : 0);
    if (err != ESP_OK) {
        Serial.printf("Error setting lock state: %d\n", err);
        return;
...
// Функция для получения глобального состояния блокировки (старая версия)
bool getGlobalLockState() {
    int8_t locked = 0;
    esp_err_t err = nvs_get_i8(nvsHandle, KEY_IS_LOCKED, &locked);
    if (err != ESP_OK && err != ESP_ERR_NVS_NOT_FOUND) {
        Serial.printf("Error getting lock state: %d\n", err);
