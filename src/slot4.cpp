/*
 * Slot 4 - Оптимизированная версия с одновременной работой HID и сканера
 * 
 * Особенности:
...
void loop() {
    ...
    if (M5.BtnB.wasPressed()) {
        if (!scanMode) {
            ...
            // 1. Настраиваем и запускаем сканирование
            NimBLEScan* pScan = NimBLEDevice::getScan();
            pScan->setActiveScan(true);
            // Добавляем фильтры для уменьшения нагрузки
            pScan->setInterval(100);      // Интервал между сканированиями
            pScan->setWindow(50);         // Время активного сканирования
            pScan->setMaxResults(10);     // Максимум 10 устройств
            pScan->start(0);
            ...
            // 2. Пересоздаем HID устройство
            Serial.println("Recreating HID device...");
            hid = new NimBLEHIDDevice(bleServer);
            hid->setManufacturer("M5Stack");
            hid->setPnp(0x02, 0x05AC, 0x820A, 0x0001);
            hid->setHidInfo(0x00, 0x01);
    ...
    if (scanMode) {
        ...
        Disbuff->setTextColor(connected ? GREEN : RED);
        Disbuff->setCursor(10, 10);
        Disbuff->printf("BLE: %s", connected ? "Connected" : "Waiting");
        
        // Показываем найденные устройства
        Disbuff->setTextColor(WHITE);
        Disbuff->setCursor(10, 40);
        Disbuff->printf("Scan: %d dev", results.getCount());
        
        // Показываем последние 3 устройства
        int y = 70;
        for(int i = 0; i < min(3, results.getCount()); i++) {
            const NimBLEAdvertisedDevice* device = results.getDevice(i);
            Disbuff->setCursor(10, y);
            Disbuff->printf("%s: %d", 
                device->getAddress().toString().substr(12, 5).c_str(),
                device->getRSSI()
            );
            y += 25;
        }
