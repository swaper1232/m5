 ...
 *   4. Запуск рекламы с правильными параметрами
 * - Не требуется:
 *   - Deinit/init стека
...
void loop() {
    ...
    if (M5.BtnB.wasPressed()) {
        if (!scanMode) {
            ...
            // 2. Останавливаем рекламу
            Serial.println("Stopping advertising...");
            bleServer->getAdvertising()->stop();
            
            // 3. Перезапускаем HID сервис
            Serial.println("Restarting HID service...");
            hid->startServices();  // Просто перезапускаем сервисы
            
            // 4. Запускаем рекламу с правильными параметрами
